...
create table public.students (id uuid primary key default gen_random_uuid(), profile_id uuid unique references public.profiles(id) on delete set null, registration_number text not null unique, first_name text not null, last_name text not null, phone text, personal_email text, programme_id uuid not null references public.programmes(id) on delete restrict, status public.student_status not null default 'active', admitted_at date not null default current_date, archived_at timestamptz, created_at timestamptz not null default now(), check((status = 'archived') = (archived_at is not null)));
